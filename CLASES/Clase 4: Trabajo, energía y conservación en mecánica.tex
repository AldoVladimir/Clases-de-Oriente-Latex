...
\begin{nota}
    \textbf{Importante:} El trabajo es una magnitud \textit{escalar} (no vectorial), aunque depende de vectores (fuerza y desplazamiento). El ángulo $\theta$ determina si el trabajo es positivo, nulo o negativo.
\end{nota}
\end{seccion}
...
\begin{seccion}
El signo del trabajo lo determina el ángulo $\theta$ entre la fuerza y el movimiento. Una regla práctica: si la fuerza \textit{ayuda} al movimiento, el trabajo es positivo; si lo \textit{frena}, es negativo; si es perpendicular, es cero.

\begin{center}
\begin{tabular}{|c|c|l|}
\hline
\rowcolor{azulmedio!30}
\textbf{Ángulo} & \textbf{Trabajo} & \textbf{Ejemplo} \\
\hline
$0^\circ \leq \theta < 90^\circ$ & $W > 0$ & Motor empuja auto hacia adelante \\
\hline
$\theta = 90^\circ$ & $W = 0$ & Cargar una caja caminando horizontalmente \\
\hline
$90^\circ < \theta \leq 180^\circ$ & $W < 0$ & Fricción frenando un objeto \\
\hline
\end{tabular}
\end{center}
\end{seccion}
...
\begin{tikzpicture}[scale=1.0]
  ...
  % Título arriba del vector, con margen holgado
  \node[font=\small\bfseries, color=azuloscuro, anchor=west] at (0, 5.2)
      {Trabajo positivo ($W > 0$)};
  \draw[thick, ->, >=Stealth] (0,3.5) -- (5,3.5) node[right, font=\small] {$d$};
  \draw[very thick, ->, >=Stealth, color=azuloscuro] (0.5,3.5) -- (3.2,4.8)
      node[right, font=\small] {$\vec{F}$};
  \draw[color=azulmedio, thin] (1.1,3.5) arc[start angle=0, end angle=27, radius=0.6cm];
  \node[color=azulmedio, font=\small] at (1.6, 3.68) {$\theta$};
  \fill[black] (0.5,3.5) circle (2.5pt);

  % --- CASO 2: Trabajo nulo ---
  \node[font=\small\bfseries, color=azuloscuro, anchor=west] at (0, 3.1)
      {Trabajo nulo ($W = 0$)};
  \draw[thick, ->, >=Stealth] (0,1.5) -- (5,1.5) node[right, font=\small] {$d$};
  \draw[very thick, ->, >=Stealth, color=orange] (0.5,1.5) -- (0.5,2.7)
      node[left, font=\small] {$\vec{F}$};
  \draw[color=orange, thin] (0.9,1.5) arc[start angle=0, end angle=90, radius=0.4cm];
  \node[color=orange, font=\small] at (1.2, 1.72) {$90^\circ$};
  \fill[black] (0.5,1.5) circle (2.5pt);

  % --- CASO 3: Trabajo negativo ---
  \node[font=\small\bfseries, color=azuloscuro, anchor=west] at (0, 0.8)
      {Trabajo negativo ($W < 0$)};
  \draw[thick, ->, >=Stealth] (0,-0.5) -- (5,-0.5) node[right, font=\small] {$d$};
  \draw[very thick, ->, >=Stealth, color=red!70!black] (4.5,-0.5) -- (2.0,-0.5)
      node[above, font=\small] {$\vec{F}$};
  \fill[black] (0.5,-0.5) circle (2.5pt);
  \node[color=red!70!black, font=\small] at (3.8,-0.15) {$\theta = 180^\circ$};
...
\textbf{Solución:}
$$W = F \cdot d \cdot \cos\theta = 100\,\text{N} \times 5\,\text{m} \times 0.5 = 250\,\text{J}$$
...
$$P = Fv \quad \Rightarrow \quad v = \frac{P}{F} = \frac{500N}{250W} = 2 \text{ m/s}$$
\end{seccion}
...
$$W = \frac{1}{2}(1\,000\,\text{kg})(20\,\text{m/s})^2 - \frac{1}{2}(1\,000\,\text{kg})(10\,\text{m/s})^2 = 200\,000\,\text{J} - 50\,000\,\text{J} = 150\,000\,\text{J}$$
\end{seccion}
...
\begin{itemize}
    ...
    \item $m$ = masa (kg)
    \item $g$ = aceleración gravitacional ($g \approx 10 \text{ m/s}^2$)
    \item $h$ = altura sobre el nivel de referencia (m)
\end{itemize}
\end{ecuacion}

\textbf{Recordar que} $E_p$ solo depende de la \textbf{altura final}, no de la trayectoria seguida para llegar ahí. Un objeto que sube por escaleras, por una rampa o en elevador llega a la misma $E_p$ si alcanza la misma altura.

\vspace{0.3cm}

\textbf{Ejemplo:} Una roca de 5 kg a 20 m de altura.
$$E_p = mgh = 5\,\text{kg} \times 10\,\text{m/s}^2 \times 20\,\text{m} = 1\,000\,\text{J}$$
\end{seccion}
...
\begin{tikzpicture}[scale=0.9]
  ...
  \draw[thick] (0,0) -- (10,0);
  \node[font=\small] at (5,-1.1) {Nivel de referencia ($h = 0$, $E_p = 0$)};
...
\textbf{Datos:} $m = 2$ kg, $h_i = 20$ m, $v_i = 0$, $h_f = 0$, $g \approx 10$ m/s$^2$
...
$$gh_i = \frac{1}{2}v_f^2 \quad \Rightarrow \quad v_f = \sqrt{2gh_i} = \sqrt{2(10\,\text{m/s}^2)(20\,\text{m})} = \sqrt{400\,\text{m}^2/\text{s}^2} = 20\,\text{m/s}$$
...
\begin{tikzpicture}[scale=1.05]
  ...
  \node[azuloscuro, font=\small, align=left, left=8pt] at (2.2,3.0) {$E_p$ m\'{a}x \\\\ $E_c{=}0$};
  ...
  \node[verdecorrecto, font=\small, align=left, right=8pt] at (4.5,2.2) {$E_c$ m\'{a}x \\\\ $E_p{=}0$};

  % Cuerda derecha
  \draw[thick, gray!70] (4.5,5.2) -- (6.8,3.0);
  \fill[azuloscuro] (6.8,3.0) circle (11pt);
  \node[white, font=\tiny\bfseries] at (6.8,3.0) {$m$};
  \node[azuloscuro, font=\small, align=left, right=8pt] at (6.8,3.0) {$E_p$ m\'{a}x \\\\ $E_c{=}0$};
...
Por conservación:
$$m_A v_A + m_B v_B = 0$$
$$(70\,\text{kg})\,v_A + (35\,\text{kg})(4\,\text{m/s}) = 0$$
$$v_A = \frac{-(35\,\text{kg})(4\,\text{m/s})}{70\,\text{kg}} = \frac{-140\,\text{kg·m/s}}{70\,\text{kg}} = -2\,\text{m/s}$$

El signo negativo indica que A sale en dirección \textbf{opuesta} a B, con rapidez de 2 m/s.

\begin{nota}
    \textbf{Verificación:} $p_A + p_B = (70)(-2) + (35)(4) = -140 + 140 = 0$ $\checkmark$
\end{nota}
\end{seccion}
...
$$E_{m,i} = mgh \approx 4\,\text{kg} \times 10\,\text{m/s}^2 \times 3\,\text{m} = 120\,\text{J}$$

\textbf{Paso 2 --- Calor disipado por fricción:}
$$Q = f \cdot d = 8\,\text{N} \times 5\,\text{m} = 40\,\text{J}$$

\textbf{Paso 3 --- Energía cinética final:}
$$E_{c,f} = E_{m,i} - Q = 120\,\text{J} - 40\,\text{J} = 80\,\text{J}$$

\textbf{Paso 4 --- Velocidad final:}
$$E_{c,f} = \frac{1}{2}mv_f^2 \quad \Rightarrow \quad v_f = \sqrt{\frac{2E_{c,f}}{m}} = \sqrt{\frac{2(80\,\text{J})}{4\,\text{kg}}} = \sqrt{40\,\text{m}^2/\text{s}^2} \approx 6.3\,\text{m/s}$$
\end{seccion}
